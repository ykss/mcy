    ...
    <div className="flex items-center gap-3 px-4 py-3.5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex gap-1 mt-0.5 flex-wrap">
          {member.position && (
            <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-[#FFCECE] text-[#C45050]">
              {member.position}
            </span>
          )}
          {(member.roles ?? []).map(role => (
            <span
              key={role}
              className={`px-2 py-0.5 rounded-full text-xs font-medium ${ROLE_STYLES[role]}`}
            >
              {role}
            </span>
          ))}
